...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				board cur_state = move->after_state();
				std::vector<int> BlankTiles;
				for ( int i=0;i<16;i++)
				{
					if ( cur_state.at(i) == 0) BlankTiles.push_back(i);
				}
				
				float total = move->reward();
				int n = BlankTiles.size();

				for ( auto it:BlankTiles)
				{
					board* tmp = new board(uint64_t(cur_state));
					tmp->set(it,1);
					total += (0.9f * estimate(*tmp))/n;
					tmp->set(it,2);
					total += (0.1f * estimate(*tmp))/n;
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		{
			auto& it = path.back();
			float error = target - (it.value() - it.reward());
			target = it.reward() + update(it.before_state(), alpha*error);
			// target = it.reward() + update(it.after_state(), alpha*error);
			debug << "update error = " << error << " for" << std::endl << it.after_state();
		}
	}
